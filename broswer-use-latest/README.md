...
You can now run the agent as an API server using FastAPI.
...
Send a POST request to `/run-agent` with a JSON payload:

```bash
curl -X POST http://localhost:8000/run-agent \
  -H 'Content-Type: application/json' \
  -d '{"task": "Go to https://apply.workable.com/isofttek-solutions-inc/j/847B7D1916/apply/, and try to fill out the form on the application page with sample data of a person named John Doe looking for a job as a software engineer"}'
```

The response will contain the result of the agent run in JSON format.
